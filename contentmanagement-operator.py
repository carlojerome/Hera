...
#Password field
        ...
        # #BOA-CTM-037 / Verify the Search button functionality without input in all search fields
        # #select operator
        # operator = wait.until(EC.visibility_of_element_located((By.CSS_SELECTOR, 'input[id="operator_id"]')))
        # operator.click()
        # time.sleep(1)
        # human_typing_action_chains(driver, operator, "eyy")
        # time.sleep(2)
        ...
        # #select currency
        # currency = wait.until(EC.visibility_of_element_located((By.CSS_SELECTOR, '#app > main > div > div > div > main > form > div > div:nth-child(2) > div')))
        # currency.click()
        # time.sleep(2)
        ...
        # #select wallet type
        # wallet_type = wait.until(EC.visibility_of_element_located((By.CSS_SELECTOR, 'div[class="grid sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-x-4"] > div:nth-child(3) > div > div')))
        # wallet_type.click()
        # time.sleep(2)
        # human_typing_action_chains(driver, wallet_type, "Transfer")
        # time.sleep(2)
        # transfer = wait.until(EC.visibility_of_element_located((By.CSS_SELECTOR, 'div[title="Transfer"]')))
        # assert transfer.is_displayed, "no Transfer in dropdown"
        # time.sleep(1)
        # transfer.click()
        # time.sleep(2)

        # #select status
        # status = wait.until(EC.visibility_of_element_located((By.CSS_SELECTOR, 'div[class="grid sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-x-4"] > div:nth-child(4) > div > div')))
        # assert status.is_displayed, "no wallet type field"
        # status.click()
        # time.sleep(2)
        # human_typing_action_chains(driver, status, "Activated")
        # time.sleep(2)
        # activated = wait.until(EC.visibility_of_element_located((By.CSS_SELECTOR, 'div[title="Activated"]')))
        # assert activated.is_displayed, "no Transfer in dropdown"
        # time.sleep(1)
        # activated.click()
        # time.sleep(2)
