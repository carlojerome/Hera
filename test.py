...
#Password field
        ...
        assert twoarrow, "no go to first page button"
        twoarrow.click()
        time.sleep(3)
        ...
        assert update, "no update button"
        update.click()
        time.sleep(2)
        ...
        des.send_keys(Keys.CONTROL + "a")
        des.send_keys(Keys.DELETE)
        human_typing_action_chains(driver, des, "test valid description")
        time.sleep(2)
        #click update
        update = wait.until(EC.visibility_of_element_located((By.CSS_SELECTOR, 'section[class="flex flex-row flex-nowrap gap-x-[20px] mt-4"] > button:nth-child(1)')))
        update.click()
        ...
        wait.until(EC.visibility_of(error2))
        assert error2.is_displayed, "no error line displayed"
        assert error2.text == "The role name must be between 3 and 100 characters.", (f"Incorrect error line! Found:{error2.text}")
        ...
        wait.until(EC.visibility_of(error2))
        assert error2.is_displayed, "no error line displayed"
        assert error2.text == "The description field is requiredd.", (f"Incorrect error line! Found:{error2.text}")
        print("BOA-PMS-040, passed")    
        time.sleep(3)
