...
#Password field
        ...
        #BOA-CTM-132 / Verify the Reset button functionality with input
        vendor_name = wait.until(EC.visibility_of_element_located((By.CSS_SELECTOR, 'input[id="vendor_name"]')))
        vendor_name.click()
        time.sleep(1)
        human_typing_action_chains(driver, vendor_name, "123asd")
        time.sleep(2)
        #select search button
        search_btn = wait.until(EC.visibility_of_element_located((By.CSS_SELECTOR, 'button[type="submit"]')))
        search_btn.click()
        time.sleep(3)
        ...
        assert len(table) >= 1, (f"Expected one or more result, but none! Found:{len(table)} entries")
        print(f"Number of entries is: {len(table)}")
        time.sleep(1)
        assert vendor_name.text.strip() == "", (f"Test failed: operator is not empty! Found: {vendor_name.get_property("value")}")
        ...
        vendor_name.click()
        time.sleep(1)
        human_typing_action_chains(driver, vendor_name, "OGP")
        time.sleep(2)
        #select search button
        search_btn = wait.until(EC.visibility_of_element_located((By.CSS_SELECTOR, 'button[type="submit"]')))
        search_btn.click()
        ...
        table = wait.until(EC.presence_of_all_elements_located((By.CSS_SELECTOR, 'tbody > tr')))
        if len(table) >= 1:
            print(f"There are data entries! found {len(table)}")
        else:
            print(f"There are no data entries! found {len(table)}")
        time.sleep(2)
        ...
        #check if the vendor name field and details in vendor name column table is the same
        assert vendor_name.text.strip() == "", (f"Test failed: operator is not empty! Found: {vendor_name.get_property("value")}")
        print(f"vendor name text is: {vendor_name.get_property("value")}")
        #table 
        table_rows = wait.until(EC.presence_of_all_elements_located((By.CSS_SELECTOR, 'tbody > tr')))

        found_vendor_table_text = True

        for row in table_rows:
            vendor_table = row.find_element(By.CSS_SELECTOR, 'tbody > tr > td:nth-child(1)')
            vendor_table_text = vendor_table.text.strip()
        
            if vendor_table_text != vendor_name.get_property("value"):
                print(f"❌Unexpected vendor found: {vendor_table_text}")
                found_vendor_table_text = False

        assert found_vendor_table_text, "not all vendor in field is the same with vendor_table_text"
        print("✅ All expected vendor text are included in the table.")
        time.sleep(2)
        print("BOA-CTM-133, passed")

        #BOA-CTM-134 / "Verify the Status with option (Existing)"
        #click the reset button
        reset = wait.until(EC.visibility_of_element_located((By.CSS_SELECTOR, 'button[type="reset"]')))
        reset.click()
        time.sleep(2)
        ...
        #for All selection
        status_page = wait.until(EC.visibility_of_element_located((By.CSS_SELECTOR, 'div[class="page"] > main > form > div > div:nth-child(2) > div > div > span:nth-child(2)')))
        status_page.click()
        time.sleep(2)
        chosen_status = status_page.text.strip()
        time.sleep(2)
        choose_status = wait.until(EC.visibility_of_element_located((By.CSS_SELECTOR, 'div[title="All"]')))
        choose_status.click()
        time.sleep(1)
        #click search button
        search_btn = wait.until(EC.visibility_of_element_located((By.CSS_SELECTOR, 'button[type="submit"]')))
        ...
        status_table = ["Activated", "Deactivated"]
        invalid_statuses = []

        table_rows = wait.until(EC.presence_of_all_elements_located((By.CSS_SELECTOR, 'tbody > tr')))

        for row in table_rows:
            status = row.find_element(By.CSS_SELECTOR, 'td:nth-child(5)')
            status_text = status.text.strip()

            if chosen_status == "All":
                if status_text not in status_table:
                    invalid_statuses.append(status_text)

        # After the loop, print invalid values if any
        if invalid_statuses:
            print("❌ Invalid statuses found in the table:")
        ...
        #######################
        #for Activated selection
        status_page = wait.until(EC.visibility_of_element_located((By.CSS_SELECTOR, 'div[class="page"] > main > form > div > div:nth-child(2) > div > div > span:nth-child(2)')))
        status_page.click()
        time.sleep(2)
        chosen_status = status_page.text.strip()
        time.sleep(2)
        choose_status = wait.until(EC.visibility_of_element_located((By.CSS_SELECTOR, 'div[title="Activated"]')))
        choose_status.click()
        time.sleep(1)
        #click search button
        search_btn = wait.until(EC.visibility_of_element_located((By.CSS_SELECTOR, 'button[type="submit"]')))
        ...
        print("✅ All the status in the table are Activated")
        time.sleep(2)
        ...
        ##############################
        #for Deactivated selection
        status_page = wait.until(EC.visibility_of_element_located((By.CSS_SELECTOR, 'div[class="page"] > main > form > div > div:nth-child(2) > div > div > span:nth-child(2)')))
        status_page.click()
        time.sleep(2)
        chosen_status = status_page.text.strip()
        time.sleep(2)
        choose_status = wait.until(EC.visibility_of_element_located((By.CSS_SELECTOR, 'div[title="Deactivated"]')))
        choose_status.click()
        time.sleep(1)
        #click search button
        search_btn = wait.until(EC.visibility_of_element_located((By.CSS_SELECTOR, 'button[type="submit"]')))
        ...
        for row in table_rows:
            ...
            if status_text != "Deactivated":
                print(f"❌Unexpected status found: {status_text}")
                found_all_status = False
        print("✅ All the status in the table are Deactivated")
        time.sleep(2)

        print("BOA-CTM-134, passed")
        ...
        #BOA-CTM-135 / Validate the Save button with no input in all fields
        ...
        #check the modal if correct
        vendor_modal = wait.until(EC.visibility_of_element_located((By.CSS_SELECTOR, 'div[class="modal-header"] > span')))
        vendor_modal.is_displayed(), "no vendor modal displayed"
        time.sleep(1)
        vendor_modal_text = vendor_modal.text.strip()
        assert vendor_modal_text == "Add Vendor", "incorrect modal header text"
        print("correct vendor modal text!")
        time.sleep(1)
        ...
        time.sleep(2)
        if ven_name_erline.text == "The vendor name field is required.":
            print("vendor name error line is correct")
        else:
            print(f"vendor name error line is incorrect! found: {ven_name_erline.text}")
        time.sleep(2)      

        #for vendor nickname error line
        ven_nickname_erline = wait.until(EC.visibility_of_element_located((By.CSS_SELECTOR, 'form[class="p-[20px] w-full overflow-y-auto"] > div:nth-child(1)  > div:nth-child(2) > div:nth-child(3) > span')))
        assert ven_nickname_erline.is_displayed, "no vendor nickname error line displayed"
        time.sleep(2)
        if ven_nickname_erline.text == "The vendor nickname field is required.":
            print("vendor nickname error line is correct")
        else:
            print(f"vendor nickname error line is incorrect! found: {ven_nickname_erline.text}")
        time.sleep(2)
        ...
        time.sleep(2)
        if ven_hosturl_erline.text == "The host field is required.":
        ...
        time.sleep(2)
        if ven_whitelist_erline.text == "The whitelist ip field is required.":
        ...
        print("BOA-CTM-135, passed")
        time.sleep(2)

        driver.refresh()
        time.sleep(4)

        #BOA-CTM-136 / Verify Add Vendor using Vendor Name (Unique)
        #click add vendor button
        vendor_btn = wait.until(EC.visibility_of_element_located((By.CSS_SELECTOR, 'button[class="btn btn-success"]')))
        time.sleep(1)
        assert vendor_btn.is_displayed(), "no add vendor button displayed"
        ...
        close_modal.click()
        time.sleep(2)
        print("BOA-CTM-136, passed")

        #BOA-CTM-137 / Verify Add Vendor using Vendor Name (Existing)
        #click add vendor button
        vendor_btn = wait.until(EC.visibility_of_element_located((By.CSS_SELECTOR, 'button[class="btn btn-success"]')))
        time.sleep(1)
        assert vendor_btn.is_displayed(), "no add vendor button displayed"
        ...
        ven_name.click()
        time.sleep(1)
        human_typing_action_chains(driver, ven_name, "ogp")
        time.sleep(1)
        ...
        assert ven_name_erline.is_displayed, "no vendor name error line displayed"
        time.sleep(2)
        if ven_name_erline.text == "The vendor name has already been taken.":
            print("vendor name error line is correct")
        else:
            print(f"vendor name error line is incorrect! found: {ven_name_erline.text}")
        time.sleep(3)
        print("BOA-CTM-137, passed")

        driver.refresh()
        time.sleep(4)

        #BOA-CTM-138 / Verify Add Vendor using Status (Activated)
        #click add vendor button
        vendor_btn = wait.until(EC.visibility_of_element_located((By.CSS_SELECTOR, 'button[class="btn btn-success"]')))
        time.sleep(1)
        assert vendor_btn.is_displayed(), "no add vendor button displayed"
        ...
        #status toggle
        # status_toggle = wait.until(EC.visibility_of_element_located((By.CSS_SELECTOR, 'div[class="modal-inner"] > form > div > div:nth-child(6) > div > div > input')))
        # time.sleep(1)
        # status_toggle.click()
        time.sleep(2)
        #check status
